...
BPMNProcess.prototype.callDefaultEventHandler = function(eventType, currentFlowObjectName, handlerName, reason, done) {
    ...
    return this.defaultEventHandler.call(this.processClient, eventType, currentFlowObjectName, handlerName, reason, done);
};

/**
 * @param {Function} callback with the elements being reset
 */
BPMNProcess.prototype.iterate = function (callback, resume) {
    var definition = this.processDefinition;
    var history = this.getHistory().historyEntries;
    var currentName = history.slice(-1)[0].name;
    var current = definition.getFlowObjectByName(currentName);
    this.getState().removeTokenAt(current);
    var current_incoming = definition.getIncomingSequenceFlows(current);
    var prev;
    for (var i = history.length - 2; i >= 0; i--) {
        var prevStep = history[i];
        if (prevStep.name !== currentName) {
            prev = definition.getFlowObjectByName(prevStep.name);
            var prev_outgoing = new Set(definition.getOutgoingSequenceFlows(prev));
            var connection = current_incoming.filter(function (x) { return prev_outgoing.has(x); });
            if (!connection.length) {
                prev_outgoing = new Set(definition.getOutgoingMessageFlows(prev));
                connection = current_incoming.filter(function (x) { return prev_outgoing.has(x); });    
                if (!connection.length) {
                    continue;
                }
            }
            callback(current, prev, this);
            if (!!resume) {
                current = prev;
                currentName = prevStep.Name
                current_incoming = definition.getIncomingSequenceFlows(current);
                if (!current_incoming) {
                    current_incoming = definition.getIncomingMessageFlows(current);
                }
            }
            else {
                return prev;
...
BPMNProcess.prototype.back = function (callback) {
    var prev = this.iterate(callback);
    var definition = this.processDefinition;
    var history = this.getHistory().historyEntries;
    var currentName = history.slice(-1)[0].name;
    var current = definition.getFlowObjectByName(currentName);
    
    if (prev.type === 'startEvent') {
        var incoming = definition.getIncomingMessageFlows(prev);
        if (!!incoming && incoming.length > 0) {
            var incomingProcessId = incoming[0].sourceProcessDefinitionId;
            var self = this;
            var prev_process = Object.keys(this.participants)
                .map(function (k) { return self.participants[k]; })
                .find(function (p) { return incomingProcessId === p.getProcessDefinition().bpmnId; });
            return prev_process.back(callback);
        }
    }
    this.getState().removeTokenAt(prev);
    this._putTokenAt(definition.getFlowObjectByName(prev.name), {});
}
