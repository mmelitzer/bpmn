...
exports.callHandler = function(name, process, data, handlerDoneCallback) {
    ...
    if (handler) {
        ...
        if (handlerType === 'function') {
            ...
            return process.callDefaultEventHandler(eventType, name, mapName2HandlerName(name),
                                            "Unknown handler type: '" + handlerType + "'", done);
        }
     } else {
        return process.callDefaultEventHandler(eventType, name, mapName2HandlerName(name), "No handler found", done);
    }
