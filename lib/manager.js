...
ProcessManager.prototype.restart = function(callback) {
    this.getAllProcesses(function(err, processes) {
        var index = processes.findIndex(function(p) {
            var isFinished = p.getHistory().isFinished();
            if (isFinished) {
                var name = p.getHistory().historyEntries.slice(-1)[0].name;
                return !p.getProcessDefinition().getFlowObjectByName(name).isMessageEvent;
            }
            return !isFinished;
        });
        if (index === -1) {
