...
ProcessManager.prototype.addBpmnFilePath = function(bpmnFilePath, processHandler){
    ...
    //     this.createProcess(processDefinitions.map(function(d) {
    //         return {
    //             name: d.name,
    //             id: d.name
    //         }
...
ProcessManager.prototype.addBpmnXML = function(bpmnXml, processName, processHandler){
    ...
    processDefinitions = definitions.getBPMNProcessDefinitionsFromXML(definitions.getBPMNDefinitionsFromXML(bpmnXml, processName));
...
/**
 * @param {Function} callback to retrieve the DB content
 */
ProcessManager.prototype.dump = function(callback) {
    this._persistency.dump(callback);
}

ProcessManager.prototype.restart = function(callback) {
    this.getAllProcesses(function(err, processes) {
        const index = processes.findIndex(function(p) {
            var entries = p.getHistory().historyEntries;
            // history.isFinished??
            // history.isFinished??
            return entries.length > 0 && entries.slice(-1)[0].type !== 'endEvent';
        });
        var process, definition = null;
        for (var i = 0; i <= index; i++) {
            //move into process and share code with back if possible??
            //move into process and share code with back if possible??
            //we have an issue with state, clean up!!
            process = processes[i]._getImplementation();
            definition = process.getProcessDefinition();
            var historyEntries = process.getHistory().historyEntries;
            var flowObjects = historyEntries.map(function(h){ return h.name; })
                .map(function(h) { return definition.getFlowObjectByName(h); }).reverse();
            
            var current_incoming, prev = null;
            flowObjects.forEach(function(current) {
                if (!!prev) {
                    if (prev === current) {
                        return;
                    }
                    else {
                        var current_outgoing = new Set(definition.getOutgoingSequenceFlows(current));
                        var prev_incoming = definition.getIncomingSequenceFlows(prev);
                        var connection = prev_incoming.filter(function(x) { return current_outgoing.has(x); });
                        if (connection.length == 0) {
                            return;
                        }
                    }
                }
                callback(prev, current, process);
                prev = current;
            });
        }
        var currentFlowObject = process.getHistory().historyEntries.slice(-1)[0];
        process.getState().removeTokenAt(currentFlowObject)
        process._putTokenAt(definition.getFlowObjectByName(currentFlowObject.name), {});
    });
}
